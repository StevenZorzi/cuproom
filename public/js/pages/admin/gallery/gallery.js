...
$(window).on('load', function() {
    ...
    $("#dropzonePreview").on('sortupdate', function() {
    	var ordering = $(this).sortable('serialize');
      	...
      	$.post(getUrl('reorder_url'), ordering).done(function(response){
      		success('ok_reorder');
      		$(".panel.preview img").attr('src', url['path_gallery_img']+"/"+response);
      	});
    });
	...
	$('#date-from .input-group.date').datepicker({
        ...
        $('#form-update-main').formValidation('revalidateField', $(this).find('input'));
    });

    $('#date-to .input-group.date').datepicker({
        language: lang,
        format: 'dd/mm/yyyy',
        todayBtn: true,
        autoclose: true,
	...
	$("#delete").on('click', function(){
        ...
        bootbox.confirm({
            title: "<h4>Eliminare questa gallery?</h4>",
            message: "<p class='text-normal'>La gallery eliminata finirà nel cestino.</p>",
            buttons: {
                confirm: {
                    label: 'Conferma',
    ...
    $('#form-add-trans').formValidation({
        ...
        fields: {
            ...
            title: {
                validators: {
                    notEmpty: {
                        message: 'Il titolo della gallery è obbligatorio'
                    }
                }
            }
        }
    ...
    .change(function(e) {
        var $form = $('#form-update-main'),
            fv = $form.data('formValidation');
        $('#form-update-main').formValidation('revalidateField', 'product[]');

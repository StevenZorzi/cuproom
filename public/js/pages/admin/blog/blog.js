...
$(window).on('load', function() {
    ...
    $("#dropzonePreview").on('sortupdate', function() {
    	var ordering = $(this).sortable('serialize');
      	...
      	$.post(getUrl('reorder_url'), ordering).done(function(response){
      		success('ok_reorder');
      		$(".panel.preview img").attr('src', url['path_blog_img']+"/"+response);
      	});
    });
	...
	$("#delete").on('click', function(){
    	...
    	bootbox.confirm({
			title: "<h4>Eliminare questo articolo?</h4>",
		    message: "<p class='text-normal'>L\'articolo eliminato finirà nel cestino.</p>",
		    buttons: {
		        confirm: {
		            label: 'Conferma',
	    ...
	    $('#form-add-trans').formValidation({
	        ...
	        fields: {
	            ...
	            title: {
	                validators: {
	                    notEmpty: {
	                        message: 'Il titolo dell\'articolo è obbligatorio'
	                    }
	                }
	            }
	        }

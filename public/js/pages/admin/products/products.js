...
$(window).on('load', function() {
    ...
    $("#dropzonePreview").on('sortupdate', function() {
    	var ordering = $(this).sortable('serialize');
      	...
      	$.post(getUrl('reorder_url'), ordering).done(function(response){
      		success('ok_reorder');
      		$(".panel.preview img").attr('src', url['path_products_img']+"/"+response);
      	});
    });
	...
	$("#delete").on('click', function(){
    	...
    	bootbox.confirm({
			title: "<h4>Eliminare questo prodotto?</h4>",
		    message: "<p class='text-normal'>Il prodotto eliminato finirà nel cestino.</p>",
		    buttons: {
		        confirm: {
		            label: 'Conferma',
    ...
    $(document).ready(function() {
    	...
    	$('#form-update-main').formValidation({
	        ...
	        fields: {
	        	...
	        	code: {
	            	validators: {
		            	stringLength: {
	                        max: 50,
	                        message: 'Il codice deve essere più corto di 50 caratteri'
	                    }
                	}
	            },
	            price: {
	            	validators: {
		            	numeric: {
	                        message: 'Il prezzo inserito non è corretto',
                            thousandsSeparator: '',
                            decimalSeparator: ','
	                    }
                	}
	    ...
	    $('#form-add-trans').formValidation({
	        ...
	        fields: {
	            ...
	            title: {
	                validators: {
	                    notEmpty: {
	                        message: 'Il titolo del prodotto è obbligatorio'
	                    }
	                }
	            }
	        }
	    ...
	    .on('success.field.fv', function(e, data) {
            ...
            $parent.removeClass('has-success');
            data.element.data('fv.icon').hide();
            //alert($(this).find('input[name=trans_id]').val());
        ...
        }).end().find('[name=data_sheet]').summernote()
        .on('summernote.change', function(customEvent, contents, $editable) {
            $('.tab-pane.active .form-update-trans').formValidation('revalidateField', 'data_sheet');
        }).end();
        ...
        .change(function(e) {
        	var $form = $('.form-update-assoc'),
                fv = $form.data('formValidation');
            $('.form-update-assoc').formValidation('revalidateField', 'brand[]');

...
$(window).on('load', function() {
    ...
    $("#dropzonePreview").on('sortupdate', function() {
    	var ordering = $(this).sortable('serialize');
      	...
      	$.post(getUrl('reorder_url'), ordering).done(function(response){
      		success('ok_reorder');
      		$(".panel.preview img").attr('src', url['path_brands_img']+"/"+response);
      	});
    });*/
	...
	$("#delete").on('click', function(){
    	...
    	bootbox.confirm({
			title: "<h4>Eliminare questo brand?</h4>",
		    message: "<p class='text-normal'>Il brand eliminato finirà nel cestino.</p>",
		    buttons: {
		        confirm: {
		            label: 'Conferma',
    ...
    $(document).ready(function() {
	    ...
	    $('#form-add-trans').formValidation({
	        ...
	        fields: {
	            ...
	            title: {
	                validators: {
	                    notEmpty: {
	                        message: 'Il nome del brand è obbligatorio'
	                    }
	                }
	            }
	        }
        ...
        .on('summernote.change', function(customEvent, contents, $editable) {
            $('.tab-pane.active .form-update-trans').formValidation('revalidateField', 'description');
        }).end();

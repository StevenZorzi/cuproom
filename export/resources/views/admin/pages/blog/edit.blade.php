...
@section('content')
	...
	<div class="row">
			...
			<div class="panel">
			    ...
			    <div class="panel-body edit-page">
				    ...
				    <div class="tab-content">
					    @foreach($translates as $trans)
						    ...
						    <div id="tab-{{$trans->lang}}" class="tab-pane fade @if($loop->first)active in @endif">
						    	...
						    	<form class="form-update-trans" action="{{route('blog.update', ['blog' => $article->id])}}" method="POST">
	              					
	              					{{ csrf_field() }}

	              					<input type="hidden" name="obj" value="{{$article->id}}">
									...
									<div class="row">
								    	<div class="col-sm-12">
								    		<div class="form-group">
								    			<input class="form-control title" name="title" placeholder="Titolo" value="{{$trans->title or 'Nuovo Articolo'}}">
								    		</div>
										</div>
									</div>
									<div class="row">
									...
									<div class="row mar-btm">
										<div class="col-sm-12">
											<textarea class="content" name="content">{!!$trans->content!!}</textarea>
											<br>
										</div>
									</div>
			 	...
			 	<div class="panel-footer text-center">
			 		<button class="delete-trans btn btn-danger btn-xs btn-rounded btn-labeled fa fa-trash" data-action="{{route('blog.destroy', ['blog' => $article->id])}}" data-lang="{{$trans->lang}}">@lang('interface.delete') @lang('interface.trans')</button>
			 	</div>
			 	@endif
			</div>
			<!-- fine pannello -->
		...
		<div class="col-md-4 others-data">
			...
			<form id="form-update-main" action="{{route('blog.update', ['blog' => $article->id])}}" method="POST">
	            
	            {{ csrf_field() }}

				<input type="hidden" name="main" value="{{$article->id}}">
				<input type="hidden" name="_method" value="PUT">
					...
					<div class="panel-body">
						<table style="width:100%">
							<tr>
								...
								<td class="text-right">
									<div class="mar-btm">
										<input name="active" id="active" class="form-control switchery" type="checkbox" value="1" @if($article->active) checked @endif >
									</div>
								</td>
							</tr>
							<tr>
								...
								<td class="text-right">
									<div id="date-created_at">
										<div class="form-group time">
		                                    <input type="text" class="pull-right mar-btm form-control text-right" name="time_created_at" placeholder="ore:min" value="{{$article->created_at->format('H:i')}}" style="width:65px; background:#fff" readonly>
		                                </div>
		                                <div class="form-group date">
		                                    <input type="text" class="pull-right mar-btm form-control text-right" name="date_created_at" placeholder="gg/mm/aaaa" value="{{$article->created_at->format('d/m/Y')}}" style="width:90px">
		                                </div>
							...
							<tr>
								...
								<td class="text-right">
									<small>{{$article->updated_at->formatLocalized('%d %h %Y %H:%M')}}</small>
								</td>
							</tr>
							<tr>
								<td>
				...
				<div class="panel">
					...
					<div class="panel-body">
						<div class="mar-btm">
							<small>Selezionare le categorie in cui far comparire questo articolo</small>
						</div>
						<div id="jstree">
				...
				<div class="panel">
					<div class="panel-heading">
						<div class="panel-control">
							<button type="button" class="btn btn-default" data-target="#demo-panel-collapse-default" data-toggle="collapse" aria-expanded="true"><i class="fa fa-chevron-down"></i></button>
						</div>
						<h3 class="panel-title">Dati Evento</h3>
					</div>
						...
						<div class="panel-body">
							<table style="width:100%">
								<tr>
									<td>
										<div class="form-group">
											<label><small>@lang('interface.date-ev-start')</small></label>
										
											<div id="date-from">
				                                <div class="input-group date" @if($article->date_from) data-date="{{ $article->date_from ? $article->date_from->format('d/m/Y') : ''}}" @endif>
				                                    <input type="text" class="form-control" name="date_from" placeholder="gg/mm/aaaa" value="{{ $article->date_from ? $article->date_from->format('d/m/Y') : ''}}">
								...
								<tr>
									<td>
										<div class="form-group">
											...
											<div id="date-to">
												<div class="input-group date" @if($article->date_to) data-date="{{ $article->date_to ? $article->date_to->format('d/m/Y'): ''}}" @endif>
				                                    <input type="text" class="form-control" name="date_to" placeholder="gg/mm/aaaa" value="{{ $article->date_to ? $article->date_to->format('d/m/Y'): ''}}">
				                                    <span class="input-group-addon"><i class="fa fa-calendar fa-lg"></i></span>
				                                </div>
				                            </div>
			                            </div>
								...
								<tr>
									<td>
										<label class="mar-top"><small>@lang('interface.time')</small></label>
									</td>
									
								</tr>
								<tr>
									<td class="text-right">
										<input type="text" class="form-control" name="time" value="{{$article->time}}">
									</td>
								</tr>
								<tr>
									<td>
								...
								<tr>
									<td class="text-right">
										<textarea rows="5" type="text" class="form-control" name="place">{{$article->place}}</textarea>
									</td>
								</tr>
							</table>
						</div>
	...
	<div class="row">
		<div class="col-md-8">
		  <div class="panel">
		    ...
		    <div class="panel-body">
		      {!! Form::open(['url' => route('blog-upload-img', ['blog' => $article->id ]), 'class' => 'dropzone', 'files'=>true, 'id'=>'real-dropzone']) !!}
		...
		<div class="col-md-4">
			...
			<div class="panel preview @if($preview == 'img/noimg.jpg') hide @endif">
			    ...
			    <div class="panel-body">
			      	<img class="img-responsive" src="{{asset($article->preview())}}">
			    </div>
			</div>
	...
	<hr>
		<div class="mar-btm mar-top text-center">
			<button id="delete" class="btn btn-danger btn-rounded btn-sm btn-labeled fa fa-trash" data-action="{{route('blog.destroy', ['blog' => $article->id])}}">@lang('interface.delete') Articolo</button>
		</div>
	</div>
	...
	<div class="modal fade" id="add-trans" role="dialog" tabindex="-1" aria-labelledby="demo-default-modal" aria-hidden="true">
	    <div class="modal-dialog">
	        <div class="modal-content">
	            <!-- BASIC FORM ELEMENTS -->
	            <!--===================================================-->
	            <form id="form-add-trans" action="{{route('blog.store')}}" method="POST">
	              	{{ csrf_field() }}
...
@section('page-script')
	...
	<script>
		var lang = "{{ $lang }}";
		var ref_id = "{{ $article->id }}";
		url['change_cat'] = "{{route('blog.update', ['blog' => $article->id])}}";
		url['path_blog_img'] = "{{url(config('paths.blog_img').$article->id)}}";
		url['get_img'] = "{{route('blog-get-img', ['blog' => $article->id])}}";
		url['get_img_info'] = "{{route('get-img-info')}}";
		url['update_img_info'] = "{{route('update-img-info')}}";
		url['delete_img'] = "{{route('blog-delete-img', ['blog' => $article->id])}}";
		url['reorder_url'] = "{{route('reorder-images')}}";
		url['update'] = "{{route('blog.update', ['blog' => $article->id])}}";
		url['list'] = "{{route('blog.index')}}";
		url['check_slug'] = "{{route('blog-check-slug', ['blog' => $article->id])}}";


		message['ok_reorder'] = "Ordinamento immagini aggiornato";
		message['ok_category'] = "Categorie aggiornate";
		message['ok_active'] = "Articolo pubblicato";
		message['no_active'] = "Articolo spostato in bozze";
	...
	{!! Html::script('js/pages/admin/blog/blog-dropzone.js') !!}
	{!! Html::script('js/pages/admin/blog/blog.js') !!}

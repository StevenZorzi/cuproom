...
<div class="row">
    <div class="col-md-7">
        ...
        <form id="form-update-user" action="{{route('users.update', ['user' => $user->id])}}" method="POST">
            <div class="panel">
                <div class="panel-heading">
                    ...
                    <h4 class="panel-title">
                        @lang('interface.info') @lang('interface.user')
                    </h4>
                </div>
                <div class="panel-body">
                        ...
                        @if(!$authUser->isUser())
                            <div class="col-sm-6">
                                <div class="form-group">
                                    <label for="role">@lang('interface.type') @lang('interface.user')</label>
                                    <select class="form-control selectpicker" name="role">
                                        <option value="user" @if($user->role == 'user') selected @endif>User</option>
                                        <option value="admin" @if($user->role == 'admin') selected @endif>Admin</option>
                                        @if($authUser->isSuperAdmin())<option value="superadmin" @if($user->role == 'superadmin') selected @endif>SuperAdmin</option>@endif
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label for="surname">@lang('interface.surname')</label>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label for="lang">@lang('interface.lang')</label>
                    ...
                    <div class="row">
                        <div class="col-sm-3">
                            <div class="form-group">
                                <label for="gender">@lang('interface.gender')</label>
                                <select class="form-control selectpicker" name="gender">
                                    <option value="M" @if($user->gender == 'M') selected @endif>Maschio</option>
                                    <option value="F" @if($user->gender == 'F') selected @endif>Femmina</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-9">
                            <div class="form-group">
                                <label for="lang">@lang('interface.timezone')</label>
    ...
    <div class="col-md-5">
        ...
        <div class="panel">
            <div class="panel-heading">
                ...
                <h4 class="panel-title text-center">
                    <i class="pli-male"></i> &nbsp; @lang('interface.user-img')
                </h4>
            </div>
            <div class="panel-body">
                ...
                <div class="col-sm-12 pad-ver">
                    {!! Form::open(['url' => route('user-upload-img', ['user' => $user]), 'class' => 'dropzone', 'files'=>true, 'id'=>'real-dropzone']) !!}
        ...
        <form id="form-change-pwd" action="{{ route('user-change-password', ['user' => $user]) }}" method="POST">
            <div class="panel">
                <div class="panel-heading">
                  
                    <h4 class="panel-title text-center">
                        <i class="fa fa-key"></i> &nbsp; @lang('interface.change-pwd')
                    </h4>
                    ...
                    <div class="col-sm-8 col-sm-offset-2">
                        ...
                        <div class="form-group">
                            <div class="input-group">
                                ...
                                <input type="password" name="pwd" class="form-control" placeholder="Nuova password">
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="input-group">
                                <div class="input-group-addon"><i class="fa fa-asterisk"></i></div>
                                <input type="password" name="confirm_pwd" class="form-control" placeholder="Conferma password">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="form-checkbox form-icon active">
                                    <input type="checkbox" name="send"> <small class="text-muted">Invia e-mail all'utente con la nuova password </small>
                            </label>
                        </div>
                ...
                <div class="panel-footer text-center">
                    <button type="submit" class="btn btn-primary btn-sm btn-rounded btn-labeled btn-control fa fa-check" disabled="">@lang('interface.confirm')</button>
                </div>
...
@can('delete', $user)
    <div class="row">
        ...
        <div class="mar-btm mar-top text-center">
            <button id="delete" class="btn btn-danger btn-rounded btn-sm btn-labeled fa fa-trash" data-action="{{route('users.destroy', ['user' => $user->id])}}">@lang('interface.disable') @lang('interface.user')</button>
        </div>
    </div>
@endcan
...
@section('page-script')
  ...
  <script> 
        url['path_users_img'] = "{{url(config('paths.users_img').$user->id)}}";
        url['get_img'] = "{{route('user-get-img', ['user' => $user->id])}}";
        url['delete_img'] = "{{route('user-delete-img', ['user' => $user->id])}}";
        url['reorder_url'] = "{{route('reorder-images')}}";
        url['check_email'] = "{{route('user-check-email', ['user' => $user->id])}}";
        url['list'] = "{{route('users.index')}}";
  ...
  <script> 
    $(document).ready(function () {
        $("#timezone").val('{{$user->timezone}}').selectpicker('refresh');
    });
  </script>  
  ...
  <style type="text/css">
        .dropzone .dz-preview .dz-image {
            ...
            display: block;
            z-index: 10; }
        .dropzone .dz-preview{ width:140px; position:relative; left:50%; margin-left: -70px;}
        .dropzone .dz-message{ margin:0; }
        .dropzone .dz-preview .dz-remove{ left: 60px; }
  </style>
    
  {!! Html::script('js/pages/admin/users/users-dropzone.js') !!}
  {!! Html::script('js/pages/admin/users/users.js') !!}

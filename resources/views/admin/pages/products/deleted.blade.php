...
@section('content')
	...
	<div class="row">
		...
		<div class="col-md-12">
			...
			<div class="mar-btm">
				<a style="line-height:32px" class="text-primary text-normal" href="{{route('products.index')}}">@lang('interface.view-actives')</a>
			...
			<div class="panel">
		      	...
		      	<div class="panel-heading">
					...
					<h3 class="panel-title">@lang('interface.list') @lang('interface.products') {{trans_choice('interface.deleted', 0)}}</h3>
				</div>
									...
									<tbody>
										@forelse($products as $product)
											<tr data-expanded="true">
												...
												<img src="{{asset($product->preview())}}" width="60" height="60"></td>
												<td class="max-width"><p class="text-overflow"><strong>{{$product->getMainText()->name}}</strong></p></td>
												<td class="max-width"><p class="text-overflow">{{$product->getMainText()->excerpt()}}</p></td>
												<td class="text-center" data-order="{{$product->created_at->toDayDateTimeString()}}">{{$product->created_at->formatLocalized('%d %h %Y %H:%M')}}</td>
												<td class="text-center">{!!$product->getStatus()!!}</td>
												<td class="text-center">
													<div class="btn-group manage" data-id="{{$product->id}}">
														<button type="button" class="restore btn btn-sm btn-default btn-icon btn-hover-info fa fa-reply add-tooltip" data-original-title="@lang('interface.restore')" data-action="{{route('products-restore', ['product' => $product->id])}}"></button>
											      
											            <button type="button" class="delete btn btn-sm btn-default btn-icon btn-hover-danger fa fa-times add-tooltip" data-original-title="@lang('interface.forcedelete')" data-action="{{route('products-destroy', ['product' => $product->id])}}"></button>
										            </div>
...
@section('page-script')
	...
	<script>
			...
			$(".manage .restore").on("click", function(){
	        	...
	        	bootbox.confirm({
					title: "<h4>{{trans_choice('interface.restore-this', 0)}} @lang('interface.product')?</h4>",
	        ...
	        $(".manage .delete").on("click", function(){
	        	...
	        	bootbox.confirm({
					title: "<h4>{{trans_choice('interface.forcedel', 0)}} @lang('interface.product')?</h4>",
